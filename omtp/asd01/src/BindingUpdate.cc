...
class BindingUpdate {
    ...
    // The IP address of the node to which a Binding Update was sent (correspondent node)
    std::string ip_cn;
    ...
    int lifetime;
    // The maximum value of the sequence number field sent in previous binding
...
public:
    ...
    BindingUpdate(std::string ip_cn, std::string ip_coa, std::string ip_ha, int lt=100) {
        setIpCn(ip_cn);
        setIpCoa(ip_coa);
        setIpHomeaddress(ip_ha);
        setInitLifetime(lt);
        generateID();
    }
    BindingUpdate(std::string ip_coa, std::string ip_ha, int lt=100) {
        setIpCoa(ip_coa);
        setIpHomeaddress(ip_ha);
        setInitLifetime(lt);
        generateID();
    ...
    void generateID() {
        id = ip_cn + ip_coa + ip_homeaddress;
    }
    ...
    const std::string& getIpCn() const {
        return ip_cn;
    }

    void setIpCn(const std::string& ipCn) {
        ip_cn = ipCn;
    }

    const std::string& getIpCoa() const {
        return ip_coa;
    }

    void setIpCoa(const std::string& ipCoa) {
        ip_coa = ipCoa;
    }

    const std::string& getIpHomeaddress() const {
        return ip_homeaddress;
    }

    void setIpHomeaddress(const std::string& ipHomeaddress) {
